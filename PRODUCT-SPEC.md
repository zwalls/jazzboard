...
- Connectors retain semantic object-ID endpoints and authoritative routing intent. A missing legacy route remains straight; new agent connectors default to obstacle-aware `auto`, whose deterministic concrete `straight`, `curved`, or `elbow` result distributes high-degree incident edges across cardinal ports, separates shared initial corridors, places labels away from known visual conflicts, and uses stable lanes while avoiding unrelated node bounds and reducing crossings. Bound endpoints created through `draw_connection` or an atomic transaction may instead specify a cardinal port side, normalized side position, and exactness when attachment meaning is deliberate. Explicit `straight`, signed-bend `curved`, and `elbow` modes plus route-relative label placement preserve deliberate visual composition. Connected geometry recomputes when nodes move, including transaction and layout operations.
...
- `analyze_diagram_layout` is a passive exact-revision read for authorized participants and spectators. It returns every resolved connector route permitted by the Diagram schema (up to 500) plus a deterministic report covering member overlap, connector intrusion/crossing/shared corridors, port congestion, label collisions, minimum spacing, and renderer-consistent likely truncation of shape labels, height-bounded text content, and connector labels. Filled-member checks follow rotated ellipse and diamond outlines and the rotated bounds of other filled objects. Open freehand strokes are not treated as filled bounding boxes; the report explicitly marks `geometryCoverage.status` `partial`, returns bounded unsupported draw IDs plus exact unsupported and omitted counts, and states that `report.status` applies only to supported geometry. Their stroke-level visual relationships remain part of preview-pixel inspection. Returned finding samples are bounded while exact aggregate counts and truncation flags remain truthful. `routeCoverage` and automatic mutation verification preserve exact omitted counts while returning only bounded ID samples and explicit sample-truncation flags. A schema-valid Diagram containing the maximum 500 members and 500 connectors remains fully previewable and PNG-exportable under the shared hard 1,000-target render cap; deterministic filled-member coverage is complete only when no unsupported freehand members are present. Every deterministic finding must be corrected and `warning` is not a pass. A `pass` with partial coverage is not complete geometry certification; the unsupported strokes must be checked in the exact preview pixels. This report is renderer-neutral geometry analysis and never claims to have inspected pixels.
...
- Agent Diagram work is closed-loop: create or edit authoritative semantic state, apply graph-aware layout and explicit ports only as appropriate, rerun `analyze_diagram_layout` on each new exact Diagram revision until it passes, render that exact passing revision, capture and inspect its pixels, and revise from the deterministic gate again when the visual result is still unreadable. An agent must not report visual completion before both the deterministic pass and actual pixel inspection.
...
- [x] Participants and spectators can run deterministic exact-revision Diagram geometry analysis without returning unrelated board state; findings cover accidental inter-member overlaps, connector conflicts, ports, labels, spacing, and likely truncation, and only `report.status: pass` clears the geometry gate. Intentional overlap within one semantic group is exempt.
...
- [x] Public agent guidance requires a closed loop—deterministic analysis until pass, exact preview rendering, browser screenshot capture, actual pixel inspection, and revision-safe iteration—and forbids claiming visual QA from preview JSON alone.
