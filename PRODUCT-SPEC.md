...
- `render_canvas_preview` lets a participant render exact object revisions or one exact Diagram revision through the authoritative first-party canvas renderer. It paints only that bounded authorized scope into a short-lived local PNG surface and returns exact viewport screenshot bounds. Current WebMCP result serialization is JSON-only, so the consuming browser host must capture that clip and a vision-capable agent must inspect the resulting pixels as a second step. When deterministic geometry coverage is partial, `geometryQualityStatus` remains `unknown` unless the supported subset already has a known `fail`. The response truthfully reports `visualInspectionStatus: not_performed`; rendering, deterministic geometry analysis, or screenshot coordinates are not visual inspection.
...
- Public Markdown guidance documents the page-scoped registration lifecycle, landing/participant/spectator tool surfaces, efficient semantic reads and transactions, revision/lease handling, the exact-code privacy boundary, and the closed Diagram-quality loop: graph-aware layout and ports, exact-revision deterministic analysis, exact preview rendering, actual browser screenshot capture, pixel inspection, and iteration.
...
- Exact preview rendering: `render_canvas_preview` for revision-guarded object or Diagram scope; its temporary in-room image is local, non-persistent, and expires automatically. The tool returns screenshot bounds and does not itself inspect pixels.
...
| Exact first-party canvas preview rendering with a temporary screenshot clip | Participant | No | Yes, local-only | No |
