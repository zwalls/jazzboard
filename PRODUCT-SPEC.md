...
- `get_canvas_capabilities` is a passive participant-and-spectator room tool with no input. It returns a versioned, renderer-neutral contract for coordinate units and axes, rotation units/direction/pivots, point spaces, named and custom colors, transparency, paint order, style metrics, supported primitives, atomic transaction operations, Diagram-membership inference, and the preferred inspection/correction loop. The current page registry and server validation remain authoritative for role permissions.
...
- Higher `zIndex` values paint in front. Equal values paint by stable object ID ascending, and omitted create z-order normally uses the current maximum plus one.
...
- `apply_canvas_transaction` can atomically create classified nodes, shapes, text, freehand drawings, structured paths, polygons, connectors, and Diagram records while revision-checking object and Diagram updates. Request-local references let relationships and Diagram membership target objects created earlier in the same call. Omitted membership arrays infer compatible objects created in the transaction; explicit arrays, including empty arrays, are exact. At most one `auto_layout` operation can arrange supported newly created references plus an optional new Diagram reference inside the same all-or-nothing commit. Existing objects use `layout_objects` with exact revisions and leases.
- `apply_canvas_transaction` also accepts an explicit draft delivery mode for genuine progressive agent work. Each call supplies the complete cumulative create-only transaction and either creates one draft or replaces its exact draft revision. Stable temporary references retain the same eventual authoritative object and Diagram IDs across stages, so visible connectors and memberships are the exact candidates that will commit rather than a cosmetic replay.
...
- Draft presentation gives the participant-owned agent an embodied, canvas-generic construction choreography without turning animation into authority. The complete authorized candidate remains immediately visible while a bounded local controller continuously travels between work areas, outlines shape and image geometry, sweeps text labels, follows authored draw paths, traces resolved connector routes, and inspects the result. Cumulative revisions append or rebase work by stable semantic ID without teleporting or replaying unchanged candidates. Playback mutates only the bot overlay transform, emits no room or network operations, caps geometry samples and queued work, cancels on lifecycle teardown, and resolves to one static inspection pose when reduced motion is requested or the page is hidden.
...
- `get_canvas_capabilities` — versioned renderer-neutral canvas coordinate, rotation, point-space, color/transparency, paint-order, primitive, transaction, and inspection contract for the current role, with compact canonical JSON examples for lifecycle-aware nodes, explicit connector ports and routing modes, direct drawing/path/polygon creation, temporary-reference transactions with inferred Diagram membership, and revision-guarded normalized path updates; it grants no authority beyond the live registry and server.
...
- Primitive canvas tools: `create_text`, `create_shape`, `create_node`, `create_drawing`, `create_path`, `create_polygon`, `add_image`, `draw_connection`, `update_object`, `move_objects`, `group_objects`, and `delete_objects`. Native paths use bounded numeric line, quadratic, and cubic segments; polygons compile to closed native paths.
- Compound semantic tools: `apply_canvas_transaction`, `layout_objects`, `create_diagram`, and `edit_diagram`.
- Progressive draft lifecycle: `apply_canvas_transaction` with `delivery.mode: "draft"` creates or replaces a complete cumulative create-only candidate at an exact draft revision; `finish_canvas_draft` commits that exact retained candidate atomically or discards an active draft.
...
| Revision-consistent semantic evidence and clean inspection framing | Participant and spectator | No | `inspect_canvas_scope` when registered | `inspect_canvas_scope` when registered |
...
- [x] Participants can validate exact revision-guarded object or Diagram scopes, frame them on the existing live first-party canvas, and use the returned short-lived screenshot clip; no duplicate modal or image surface is painted, the clip becomes stale on viewport or window changes, the response states that visual inspection was not performed, spectators receive no preview tool, and no preview image is persisted or exposed by URL.
- [x] Public agent guidance requires an intent-led closed loop—semantic reads, optional conventional geometry analysis, exact live-canvas framing, immediate browser screenshot capture, actual pixel inspection, and revision-safe iteration—and forbids claiming visual QA from preview JSON alone.
