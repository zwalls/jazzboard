...
- A frozen visual is a local PNG download. The authorized client renders the requested board, Diagram, or selection directly through tldraw so images and annotations are preserved as pixels. Jazzboard does not upload, persist, index, host, or issue a URL for the PNG.
...
- `export_canvas_png` — render an exact revision-guarded room, Diagram, or object set through the authorized local tldraw canvas and download an image-faithful PNG without server persistence.
...
- The server remains authoritative for membership, guest sessions, roles, permissions, agent edit policy, proposal review, object and Diagram revisions, lifecycle metadata, leases, transactions, activity guards, legacy snapshot expiry/revocation, template instantiation, and attribution.
...
- Portable artifacts are projections, never authorization records. Safe JSON/Mermaid/SVG rendering and fresh-ID template planning live outside tldraw. The separate visual PNG path uses the authorized tldraw renderer locally, includes renderable images, and never persists the output.
...
- **Unsafe semantic export features:** semantic artifacts never embed private/external image URLs, executable Mermaid directives, or scriptable/networked SVG features. The local visual PNG is rendered from the authorized tldraw canvas and may contain image pixels, but it is never uploaded or exposed as a Jazzboard URL. The separate exact-scope preview tool remains ephemeral visual verification, not a portable export.
...
- [x] Semantic JSON, directive-free Mermaid, and fixed-vocabulary SVG exports redact identity/session/presence/lease/media secrets; local PNG download uses the authorized tldraw renderer, preserves renderable images, and creates no Jazzboard record or URL.
