...
Spectators receive the authorized read/export rows above and no shared-state mutation tools. They can inspect activity and proposals, return a redacted semantic artifact, and request an image-faithful local PNG download, but they cannot revert, create or instantiate a template, change review policy, or operate canvas/collaboration state. No role can issue a new hosted read-only snapshot. Passive reads and the local-only download do not activate an agent or create fake presence. The first successful shared-state mutation or shared agent-viewport operation activates the current session's agent and keeps its activity attributable; private Follow changes and leaving the local room view do not.
...
8. Follow the other participant's agent and exercise Spotlight start, join/leave, request, approve/dismiss handoff, and stop through WebMCP.
9. Join a third browser as a spectator and verify its exposed WebMCP surface contains passive reads and authorized local exports but no shared-state mutation or role-upgrade tool.
...
- Spectators stay non-editing. Their semantic/PNG exports and review/activity reads do not grant canvas, policy, template, collaboration, session, or role mutations.
...
- `src/lib/webmcp`: pre-hydration landing; role-scoped semantic, activity, review, interchange, lifecycle, transaction, layout, Diagram, and exact canvas-preview tools plus passive compatibility tools on existing snapshot pages, registered through `document.modelContext.registerTool`.
