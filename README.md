...
The generated `/webmcp.md` and downloadable skill take their landing, participant, spectator, and legacy snapshot-page inventories and counts directly from executable WebMCP constants. Tests fail if those public inventories drift from registration. After room authorization, agents call `get_canvas_capabilities` before authoring; its typed response defines the renderer-neutral coordinate, rotation, palette, transparency, paint-order, primitive, transaction, and inspection contract and supplies canonical compact JSON examples for lifecycle-aware nodes, explicit connector ports/routes, drawings, native paths/polygons, inferred-membership transactions, and revision-guarded normalized path updates. The live page registry remains authoritative for role-scoped tool availability.
...
After the deterministic gate, prefer `inspect_canvas_scope` when it is registered. It returns one exact-revision bundle containing scoped semantic objects, deterministic evidence, clean live-canvas framing, and the viewport `screenshotClip`. The compatible split workflow remains `analyze_diagram_layout` followed by participant-only `render_canvas_preview`. Neither workflow sees pixels for the agent. While the returned validation selector remains active, a browser host should capture the full clean viewport and then crop those pixels to `screenshotClip`; this avoids transient geometry changes caused by some browsers' direct clipped-capture implementations. A vision-capable agent must inspect the cropped pixels. Any canvas viewport, browser-window, or scoped-revision change makes the clip stale. Validation, framing, receiving JSON, or receiving screenshot coordinates does not perform visual QA. If the pixels are unreadable, edit the semantic composition and repeat until the final exact-revision pixels are verified. The inspection tools persist no image or preview URL. This workflow is separate from the unchanged image-faithful local PNG export.
