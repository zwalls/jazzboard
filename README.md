...
After the deterministic gate, a participant calls `render_canvas_preview` for that same exact Diagram revision. It validates that authorized revision, frames the scope on the existing live canvas, and returns its exact viewport `screenshotClip` without opening a duplicate modal, painting a temporary image surface, or publishing that local camera through multiplayer presence. Stop Follow or leave Spotlight before framing. A browser host must capture the short-lived clip immediately and a vision-capable agent must inspect the returned pixels; any canvas viewport, browser-window, or scoped-revision change makes the clip stale. Jazzboard revalidates the scope after framing and restores the prior camera if framing fails or is cancelled. Partial geometry coverage keeps `geometryQualityStatus` `unknown` unless supported geometry already has a known `fail`; the result always keeps `visualInspectionStatus: not_performed`. Validation, framing, receiving JSON, or receiving screenshot coordinates does not perform visual QA. If the pixels are unreadable, edit the semantic diagram and repeat from analysis. The tool persists no image, exposes no preview URL, and remains unavailable to spectators. This inspection workflow is separate from the unchanged image-faithful local PNG export.
...
4. Give one decision and one open question structured status/owner/resolution state. Find the Diagram semantically, read one neighborhood, and apply `layout_objects`. Run `analyze_diagram_layout` on every resulting exact revision, correct every deterministic finding, and repeat until `report.status` is `pass`. Then call `render_canvas_preview` for that exact revision, immediately capture its returned live-viewport screenshot clip before the viewport or window changes, actually inspect the pixels, and iterate again if the hierarchy, whitespace, routes, endpoints, or labels remain unreadable.
