...
Spectators receive the authorized non-editing read/export rows above and no shared-state mutation tools. They can inspect activity and proposals, return a redacted semantic artifact, and request an image-faithful local PNG download, but they cannot access Ask messages, revert, create or instantiate a template, change review policy, or operate canvas/collaboration state. No role can issue a new hosted read-only snapshot. Passive reads and the local-only download do not activate an agent or create fake presence. The first successful shared-state mutation or shared agent-viewport operation activates the current session's agent and keeps its activity attributable; private Follow changes and leaving the local room view do not.
...
Agent-created connectors default to obstacle-aware `auto` routing. The server deterministically distributes high-degree incident ports, chooses readable elbow corridors, avoids shared initial paths and label conflicts, and uses stable parallel lanes, returning both the requested `routing.mode` and concrete rendered `routing.kind`. Bound endpoints in `draw_connection` and atomic connector creates may instead supply `port.side` (`top`, `right`, `bottom`, or `left`), optional `port.position`, and optional `port.exact`; explicit `straight`, signed-bend `curved`, or `elbow` routing plus route-relative label placement remains available when visual intent should override avoidance. Human bend, elbow, and attachment edits round-trip into authoritative state without snapback. Routing and endpoint metadata survive Diagram reads, activity/review/revert, templates, semantic JSON, persistence, and multiplayer projection. SVG faithfully renders resolved route geometry and labels; Mermaid intentionally preserves topology rather than visual route geometry.

Readable agent-authored diagrams use a closed verification loop. Call `analyze_diagram_layout` with one exact Diagram revision; it returns every resolved connector route allowed by the Diagram schema (up to 500) plus deterministic findings and metrics for overlaps, connector intrusions and crossings, shared corridors, congested ports, label collisions, spacing, and renderer-consistent likely truncation of shape labels, height-bounded text content, or connector labels. Filled-member checks follow rotated ellipse and diamond outlines and the rotated bounds of other filled objects. Open freehand strokes are not misclassified as filled bounding boxes: the report sets `geometryCoverage.status` to `partial`, returns bounded `unsupportedDrawObjectIds`, preserves an exact unsupported-draw metric, and states that `report.status` applies only to supported geometry. Finding samples remain bounded while aggregate counts stay exact. `routeCoverage` returns exact totals plus a bounded omitted-ID sample and truncation flag for defensive legacy-invalid-state handling; automatic mutation verification likewise bounds omitted Diagram IDs while preserving exact counts. One exact Diagram preview or PNG export can render all 500 members plus all 500 connectors under the shared hard 1,000-target cap. Correct every finding and repeat after each applied revision until `report.status` is exactly `pass`; `warning` is not a pass. When coverage is partial, a pass is not complete geometry certification and the exact preview pixels must cover the unsupported strokes. The analyzer is a passive geometry gate available to authorized participants and spectators, not a substitute for vision.
...
3. Use `apply_canvas_transaction` with one `auto_layout` operation to create and comfortably arrange a titled architecture diagram with at least four explicitly classified nodes, a crossing-prone blocker, and labeled auto, curved, and elbow connectors in one commit. Use graph-aware hierarchy layout for its acyclic relationship graph; let auto routing distribute ports except where an explicit endpoint port conveys intent.
