...
| Surface | Tools |
| --- | --- |
| Canvas capability contract | `get_canvas_capabilities` |
...
| Participant object editing | `create_text`, `create_shape`, `create_node`, `create_drawing`, `create_path`, `create_polygon`, `add_image`, `draw_connection`, `update_object`, `move_objects`, `group_objects`, `delete_objects` |
...
| Shared read and inspection | `get_canvas_capabilities`, semantic room/Diagram reads, `analyze_diagram_layout`, `inspect_canvas_scope` |
| Participant legacy visual framing | `render_canvas_preview` |
...
The generated `/webmcp.md` and downloadable skill take their landing, participant, spectator, and legacy snapshot-page inventories and counts directly from executable WebMCP constants. Tests fail if those public inventories drift from registration. After room authorization, agents call `get_canvas_capabilities` with `{}` for its compact schema-v2 core, then request exactly one relevant `authoring`, `architecture`, `illustration`, or `inspection` bundle. Task bundles progressively disclose high-signal guidance and canonical examples without forcing a monolithic manual into every task. They are guidance, not permissions: the live page registry and server remain authoritative for role-scoped tool availability and accepted mutations.
...
Jazzboard canvas geometry uses unbounded canvas units: x increases right, y increases down, and rectangular object x/y identify the unrotated top-left. Rotation is measured in radians, with positive values appearing clockwise on screen. Rectangular objects, paths, and polygons rotate around their center; legacy freehand drawings rotate around their object-local origin. Drawing, path, and polygon create inputs use absolute canvas coordinates. Authoritative drawing points use object-local canvas units; path and polygon points are normalized object-local coordinates from 0 to 1 across their bounds. Higher `zIndex` values paint in front; equal values paint by object ID ascending.

The visual vocabulary accepts Jazzboard named colors plus `#RGB`, `#RRGGBB`, and `#RRGGBBAA`. Named colors resolve to saturated ink and related pastel fills. `none` makes an eligible fill transparent, alpha hex preserves per-color transparency, and paths/polygons support opacity from 0 to 1, numeric stroke width, line caps, line joins, and fill rules. Unsupported colors fail validation rather than silently rendering as another color.

A `Diagram` is a first-class authoritative record with a stable ID, title, description, explicit type/category, tags, member object IDs, connector IDs, computed bounds, revision, timestamps, creator, and last editor. Every canvas object can also carry an explicit `semanticName` for one exact named part and `semanticRole` for its agent-readable function; these are authoritative semantic fields, not labels inferred from pixels or style, and can be queried, revised, or cleared. Transaction creates derive a missing `semanticName` from `tempRef`, but agents should supply meaningful names and roles when part identity matters. Shapes separately carry an explicit `nodeType` (`service`, `component`, `requirement`, `decision`, or `open_question`); style is never used as the source of classification. Decisions carry structured `proposed`, `accepted`, `rejected`, or `superseded` status, while open questions carry `open`, `answered`, `deferred`, or `closed`. Both support owner and resolution fields, and the server manages `resolvedAt` when workflow state resolves.

Use `apply_canvas_transaction` to create a multi-object diagram and its labeled connectors as one coherent all-or-nothing candidate. For every user-visible new multi-object composition, the standard path is `delivery: { "mode": "draft" }`: collaborators see the bot trace each candidate part before one atomic finish. Omit draft delivery only for revision-checked corrections, explicitly instant work, or work with no live audience. The tool supports nodes, shapes, text, drawings, structured line/quadratic/cubic paths, polygons, connectors, and Diagrams. Request-local `tempRef` values can connect objects and populate Diagram membership before stable IDs exist; the response returns the resolved IDs. If membership arrays are omitted, compatible objects created in the same transaction are inferred; an explicit array, including `[]`, is exact. One optional `auto_layout` operation can arrange supported new references and a new Diagram reference within that same commit. Existing objects use the separate revision- and lease-checked `layout_objects` tool. Any invalid reference, stale revision, busy object, authorization failure, or bad membership rejects the complete transaction.

`apply_canvas_transaction` and `layout_objects` default to `responseDetail: "concise"`: their bounded receipts return the outcome, exact revisions and IDs, compact named-object/Diagram records, validation counts, and an exact `recommendedInspection` call when an applied change can be inspected. Request `responseDetail: "detailed"` only when full changed entities and complete quality reports are needed. Likewise, `query_objects` and `read_neighborhood` default to `detail: "summary"`; opt into `detail: "full"` only for authoritative full objects.

For every user-visible new multi-object composition, add `delivery: { "mode": "draft" }` to `apply_canvas_transaction`. The server persists a non-authoritative preview, broadcasts only compact invalidations, and has authorized room clients fetch the renderable preview rather than simulating shared activity with timers. The first response returns a draft ID, draft-local revision, and stable `tempRef` map. Each replacement must resend the complete cumulative create-only operation list with that exact draft ID and revision; the tool first reads the authorized draft and reuses its candidate IDs. Submit the largest coherent candidate that fits the transaction limits, and replace it as quickly as semantic reasoning requires—never split or pause agent work merely to pace animation. The client presentation queue absorbs rapid revisions. A `tempRef` remains reserved for the draft's lifetime: omitting its create operation removes that candidate from the current preview, while reintroducing it later restores the original candidate ID. `read_canvas_drafts` remains available for semantic inspection, but is not a timing loop. The participant owner calls `finish_canvas_draft` once with the latest exact revision; Jazzboard renews the draft, waits internally for that revision's visible construction, and then commits atomically. Active owned drafts also receive bounded browser keepalives, with a five-minute idle window and a 30-minute hard ceiling that meaningful replacements reset. Drafts are excluded from authoritative object queries, activity, templates, semantic interchange, and PNG export until an applied commit. Existing-object and existing-Diagram edits still use the direct revision- and lease-checked transaction path. In review mode, committing returns `proposed` and keeps the draft visible as awaiting review until a human decides it. Spectators may read authorized drafts but receive no draft mutations.
...
After an applied transaction or layout, prefer its `recommendedInspection` call to `inspect_canvas_scope`. The returned `sceneContext` is schema v2 and multi-resolution: `overview` gives bounded scope aggregates, `working_set` adds compact named-object and route records, and `focus` adds full semantics only for bounded `focusObjectIds`. A visual contract can carry intent, criteria, and parts to preserve. Stable current `findingKeys` can be passed back as `previousFindingKeys`, but those keys are explicitly caller-supplied and unverified. `findingComparison` reports keys observed again, observed without a supplied match, and—only with complete current coverage—same-scope caller keys not observed; it never claims that an unobserved or invented key was authoritatively resolved. Scope and finding identities include room, Diagram, and object `createdAt` incarnations, so delete/recreate cannot carry history across reused IDs. Compact Diagram memberships return at most 16 IDs plus exact counts and a full-set digest. `boundsOverlaps` are conservative axis-aligned renderer-bounds facts, not proof of painted intersection, occlusion, or a defect, and are intentionally excluded from finding comparisons. `working_set` and `focus` also return exact-revision `textOcclusionRisks` when paint order and an opaque rectangle likely cover shared-layout text bounds; agents must inspect pixels and intent before using the included corrective choices. The compatible split workflow remains `analyze_diagram_layout` followed by participant-only `render_canvas_preview`.

Neither inspection workflow sees pixels for the agent. `sceneContext.pixels` truthfully reports `delivery: "host_capture_required"`, no native image result, and `visualInspectionStatus: "not_performed"`. While its validation selector remains active, the browser host captures the full clean viewport and crops those pixels to `screenshotClip`; a vision-capable agent judges the crop against the user's intent. Create, inspect, patch only identified defects, and repeat until success or bounded stagnation—never until a deterministic heuristic overrides deliberate composition. Any viewport, window, or scoped-revision change makes the clip stale, and the tools persist no image or preview URL.
...
3. Use `apply_canvas_transaction` with `delivery.mode: draft` to build a titled architecture diagram progressively. Replace it with the complete cumulative create-only operation list and exact draft revision as more nodes and connectors appear, optionally inspect it with `read_canvas_drafts`, then call `finish_canvas_draft` once; the finish tool waits for visible construction internally. Include at least four explicitly classified nodes, a crossing-prone blocker, labeled auto, curved, and elbow connectors, and one `auto_layout` operation. Use graph-aware hierarchy layout for its acyclic relationship graph; let auto routing distribute ports except where an explicit endpoint port conveys intent.
4. Give one decision and one open question structured status/owner/resolution state. Find the Diagram semantically, read one neighborhood, and apply `layout_objects`. Run `analyze_diagram_layout` on every resulting exact revision, correct every deterministic finding, and repeat until `report.status` is `pass`. Then call `render_canvas_preview` for that exact revision; while validation is active, capture the full clean viewport, crop those pixels to the returned screenshot clip, inspect the crop, and iterate again if hierarchy, whitespace, routes, endpoints, or labels remain unreadable.
