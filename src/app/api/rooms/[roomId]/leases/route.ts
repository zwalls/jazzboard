...
const humanLeaseSchema = z.discriminatedUnion("action", [
  z.object({
...
export async function POST(request: Request, context: Context): Promise<Response> {
  try {
    ...
    const body = humanLeaseSchema.parse(await readJsonBody(request));
    const result = await runLeaseAction({ ...body, roomId, participantId, actorKind: "human" });
