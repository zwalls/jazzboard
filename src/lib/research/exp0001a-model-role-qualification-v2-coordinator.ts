...
function authorPrompt(task: z.infer<typeof qualificationV2PublicTaskInputSchema>, privateRoomInviteUrl: string) {
  ...
  return [
    ...
    "Before browser work, satisfy the platform skill requirement with exactly one command: read the complete browser:control-in-app-browser SKILL.md from the exact filesystem path shown for that skill in your available-skills catalog. Use sed from line 1 through at least line 150, do not guess or probe any alternate path, and do not run any other command.",
...
function buildAction(input: Readonly<{
  ...
  const content = {
    ...
    arguments: {
      ...
      title: `Qualification ${input.taskId} ${input.role} ${input.roleOrdinal} ${suffix}`,
    },
    sourceTaskId: null,
