...
export const qualificationV2AuthorEvidenceSchema = z.object({
  ...
  authorSessionIdentity: z.object({
    ...
    joinResultDigest: digestSchema.nullable(),
    collaborationResultDigest: digestSchema.nullable(),
    authoritativeSessionBindingDigest: digestSchema.optional(),
    bindingDigest: digestSchema,
  }).strict(),
  ...
  nestedWebMcpResultObservation: z.enum(["observed", "unobservable"]).optional(),
  sessionBindingMethod: z.enum(["retained_result_marker", "authoritative_participant_delta"]).optional(),
  visualProofMethod: z.enum([
    "retained_result_marker",
    "completed_bound_program_plus_controller_capture",
  ]).optional(),
...
function reviewerPrompt(input: Readonly<{
  ...
  return [
    ...
    "After the browser-skill read, run exactly the following two node_repl JavaScript invocations, in order, and no other node_repl invocation. Preserve the variable names and statements exactly; replace nothing:",
    `BROWSER_SETUP_JS=const { setupBrowserRuntime } = await import("/Users/zacharywalls/.codex/plugins/cache/openai-bundled/browser/26.825.51511/scripts/browser-client.mjs"); const browserAgent = await setupBrowserRuntime(); const browser = await browserAgent.browsers.getForUrl(${JSON.stringify(input.exactRevisionPngUrl)}); nodeRepl.write(await browser.documentation());`,
    `IMAGE_INSPECTION_JS=const tab = await browser.tabs.new(); await tab.goto(${JSON.stringify(input.exactRevisionPngUrl)}); await tab.playwright.waitForLoadState({state:"load",timeoutMs:10000}); const evidencePixels = await tab.screenshot({fullPage:true}); await nodeRepl.emitImage(evidencePixels);`,
    "Do not call agent.documentation.get, use an iab/chrome/edge binding, inspect capabilities, evaluate the page, encode image bytes, call nodeRepl.write on the screenshot, navigate again, or compute a digest. The evidence sidecar permits exactly one successful image read.",
...
function buildAction(input: Readonly<{
  ...
  const content = {
    ...
    arguments: {
      prompt: input.prompt,
      target: { type: "projectless" as const, directoryName: `qual-${input.role.replaceAll("_", "-")}-${suffix}` },
