...
export function makeLlmsTxt(origin = JAZZBOARD_ORIGIN): string {
  return document([
    ...
    "Choose exact placement, optional layout, connector routes, and ports from the user's intended composition. For a conventional Diagram-readability goal, use `analyze_diagram_layout` as intent-unaware evidence and correct unintended findings before sign-off. For illustration or freeform work, preserve deliberate geometry. Call `render_canvas_preview`, capture its `screenshotClip`, and actually inspect the final pixels; preview rendering alone is not visual inspection.",
    "",
    "## Start here",
    "",
...
export function makeHomepageMarkdown(origin = JAZZBOARD_ORIGIN): string {
  return document([
    ...
    "Visual creation is an intent-led closed loop. Read exact geometry, use `analyze_diagram_layout` as intent-unaware conventional-diagram evidence when appropriate, and correct only findings that conflict with the requested composition. Preserve deliberate overlap, cropping, crossings, and spacing in illustration or freeform work. Call `render_canvas_preview` for the exact revision, capture its `screenshotClip`, and inspect the actual pixels; rendering does not perform or certify visual inspection.",
    "",
...
export function makeSkillMarkdown(origin = JAZZBOARD_ORIGIN): string {
  return document([
    ...
    "- After any geometry review, call `render_canvas_preview` for that exact Diagram revision, capture the returned `screenshotClip` before expiry with the browser host, and inspect the actual pixels against the requested intent. Partial coverage keeps `geometryQualityStatus` unknown unless a supported-geometry failure is already known. The preview JSON truthfully reports `visualInspectionStatus: not_performed`; rendering is not inspection.",
