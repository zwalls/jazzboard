...
export function makeAgentGuideMarkdown(origin = JAZZBOARD_ORIGIN): string {
  return document([
    ...
    "",
    "Use an explicit `straight`, `curved`, or `elbow` routing mode when the composition requires a particular path. `curved` requires a signed `bend`; `elbow` accepts an `elbowMidPoint` from 0 to 1; every mode may set `labelPosition` from 0 to 1. Semantic reads report endpoint `normalizedAnchor`, `isPrecise`, `isExact`, and `snap` metadata so an agent can understand the authoritative attachment ports. Human-authored visual-canvas port edits round-trip and promote an automatic route to an explicit route; agent mutations control route intent through the routing fields. Updating an existing connector still requires its exact revision and any active lease, including inside a transaction.",
    "",
