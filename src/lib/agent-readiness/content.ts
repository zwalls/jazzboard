...
export function makeLlmsTxt(origin = JAZZBOARD_ORIGIN): string {
  return document([
    ...
    "Choose exact placement, optional layout, connector routes, and ports from the user's intended composition. For a conventional Diagram-readability goal, use `analyze_diagram_layout` as intent-unaware evidence and correct unintended findings before sign-off. For illustration or freeform work, preserve deliberate geometry. Call `render_canvas_preview`, then immediately capture its short-lived live-canvas `screenshotClip` before the viewport or window changes and actually inspect the final pixels; framing alone is not visual inspection.",
    "",
    "## Start here",
    "",
...
export function makeHomepageMarkdown(origin = JAZZBOARD_ORIGIN): string {
  return document([
    ...
    "## Semantic canvas",
    "",
    "Agents read stable object and optional Diagram IDs, exact geometry, semantic relationships, revisions, and attribution. They can create rectangle, ellipse, and diamond shapes plus text, freehand drawings, accessible images, and connectors. Omitted connector routing delegates to `auto`; explicit ports and straight, curved, or elbow paths preserve deliberate composition. Generated singleton endpoints center on their natural side, while only automatic edges naturally sharing one side are distributed. Exact placement, overlap, drawing, and grouping remain available for illustrations and other freeform work. Graph-aware hierarchy layout is an opt-in diagram operation, not a board-wide rule.",
    ...
    "Visual creation is an intent-led closed loop. Read exact geometry, use `analyze_diagram_layout` as intent-unaware conventional-diagram evidence when appropriate, and correct only findings that conflict with the requested composition. Preserve deliberate overlap, cropping, crossings, and spacing in illustration or freeform work. Call `render_canvas_preview` for the exact revision, then immediately capture its short-lived live-canvas `screenshotClip` before the viewport or window changes and inspect the actual pixels; validation and framing do not perform or certify visual inspection.",
    "",
...
export function makeAgentGuideMarkdown(origin = JAZZBOARD_ORIGIN): string {
  return document([
    ...
    "4. Call `render_canvas_preview` for that exact Diagram revision. Jazzboard validates the authorized revision and frames it on the existing live canvas without painting a duplicate modal or temporary image surface. Partial coverage keeps `geometryQualityStatus` unknown unless supported geometry already has a known fail. The JSON result may repeat the deterministic report, but `visualInspectionStatus: not_performed` remains truthful: framing a preview is not seeing it.",
    "5. Immediately use the browser host's screenshot capability to capture exactly `screenshotClip`, then inspect the returned pixels for legibility, hierarchy, whitespace, routes, endpoints, labels, and unintended occlusion. The short-lived clip becomes stale after a canvas viewport or browser-window change. Do not claim visual QA from the tool JSON or screenshot coordinates alone.",
...
export function makeWebMcpMarkdown(origin = JAZZBOARD_ORIGIN): string {
  return document([
    ...
    "",
    "After interpreting any geometry findings against the requested intent, a participant calls `render_canvas_preview` for that same exact Diagram revision. The tool validates and frames the scope on the existing live canvas, then returns `screenshotClip` without painting a duplicate preview surface. The browser host must capture that short-lived clip immediately, before any canvas viewport or browser-window change makes it stale, and a vision-capable agent must inspect the resulting pixels. If the visual check finds unintended unreadability, weak hierarchy, poor whitespace, crossings, occlusion, or misleading endpoints, edit and inspect again. Never report visual QA from `geometryQualityStatus`, `visualQuality`, `visualInspectionStatus`, screenshot coordinates, or the preview JSON alone.",
    "",
    ...
    "| Render an exact participant scope for inspection | `render_canvas_preview` |",
    "| Actually inspect the rendered pixels | Capture the returned `screenshotClip` with the browser host, then use vision |",
...
export function makeGlossaryMarkdown(origin = JAZZBOARD_ORIGIN): string {
  return document([
    ...
    "- **Visual inspection:** A separate vision step performed after `render_canvas_preview`: immediately capture the exact returned live-canvas `screenshotClip` before a viewport or window change makes it stale, then inspect those pixels. Revision validation, framing, or receiving clip coordinates is not inspection.",
...
export function makeAgentsMarkdown(origin = JAZZBOARD_ORIGIN): string {
  return document([
    ...
    "## Usage",
    "",
    `On the landing page, call \`create_room\` or \`join_room\` with the exact user-supplied code. In a room, start with a bounded semantic read, preserve exact revisions, and inspect whether an agent mutation was applied live or returned a proposal for human review. Pull private Ask messages with \`list_agent_messages\`; claim before acting, refresh authoritative revisions, and reply with the claim token. Polling does not wake the agent. Choose exact placement, optional layout, connector routes, and ports from the requested composition; omission delegates routing to \`auto\`. Use \`read_diagram\` plus \`analyze_diagram_layout\` as intent-unaware evidence for conventional diagram work, but preserve deliberate freeform geometry. Call \`render_canvas_preview\`, immediately capture its short-lived live-canvas \`screenshotClip\` before the viewport or window changes, and inspect the final pixels; framing is not visual inspection. Use activity guards for compensating reverts, redacted semantic exports for portability, fresh-ID templates for reuse, and \`export_canvas_png\` for an image-faithful local download. Share collaboration through the exact room invitation; Jazzboard issues no new hosted snapshot URLs. A spectator has only the ${ROOM_SPECTATOR_TOOL_NAMES.length} non-editing tools registered by the executable spectator surface and no Ask inbox. Never search for rooms or infer permissions from documentation; the registered live tools and server responses are authoritative.`,
    "",
...
export function makeSkillMarkdown(origin = JAZZBOARD_ORIGIN): string {
  return document([
    ...
    "- After any geometry review, call `render_canvas_preview` for that exact Diagram revision, then immediately capture the returned live-canvas `screenshotClip` with the browser host before any viewport or window change makes it stale, and inspect the actual pixels against the requested intent. Partial coverage keeps `geometryQualityStatus` unknown unless a supported-geometry failure is already known. The preview JSON truthfully reports `visualInspectionStatus: not_performed`; validation and framing are not inspection.",
