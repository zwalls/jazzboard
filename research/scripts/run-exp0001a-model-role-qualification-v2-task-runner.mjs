...
async function runtime() {
  runtimePromise ??= (async () => {
    ...
    if (result.outputFiles?.length !== 1) throw new Error("QUALIFICATION_V2_TASK_RUNNER_BUILD_FAILED");
    await mkdir(PRIVATE_RUNTIME_ROOT, { recursive: true, mode: 0o700 });
    const directory = await mkdtemp(path.join(PRIVATE_RUNTIME_ROOT, ".task-runner-runtime-"));
...
export async function runQualificationV2TaskRunner(argv = process.argv.slice(2), io = {
  ...
  try {
    ...
    return await loaded.module.runQualificationV2TaskRunnerCli(argv, io, REPOSITORY_ROOT);
  } catch {
